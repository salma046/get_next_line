...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/11 13:16:30 by salaoui           #+#    #+#             */
/*   Updated: 2024/01/11 13:36:21 by salaoui          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buffer)
		return (NULL);
	line[fd] = ft_read_line(fd, line[fd], buffer);
	if (!line[fd])
		return (NULL);
	result = save_line(line[fd]);
	line[fd] = save_next_line(line[fd]);
	return (result);
}
